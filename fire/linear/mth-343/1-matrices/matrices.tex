...
\begin{itemize}
  \item []
    ...
    \minor{\item[19.] A fourth way to multiply matrices is columns of \tbm{A} times rows
      ...
      Give a \(2\times\) example of this important rule for matrix multiplication.}

    \end{itemize}

    \subsection{Problems 29--31}
    \minor{\begin{enumerate}\setcounter{enumi}{28}
        \item \(\downarrow\)
      \begin{enumerate}
        \item \(\bm{E}_{21}\) subtracts row 1 from row 2 and then
            \(\bm{P}_{23}\) exchanges rows 2 and 3. What matrix \(\bm{E} = \bm{P}_{23}\bm{E}_{21}\)
            does both steps at once?

          \item \(\bm{P}_{23}\) exchanges rows 2 and 3 and then \( \bm{E}_{31} \) subtracts row 1 from row 3. What
            matrix \(\bm{M} = \bm{E}_{31}\bm{P}_{23}\) does both steps at once?
            Explain why the \tbm{M}'s are the same
      \end{enumerate}
      ...
      \end{bmatrix}.
        \]
      \item This \(4\times 4 \) matrix needs which elimination matrices \( \bm{E}_{21} \) and \( \bm{E}_{32} \) and \(\bm{E}_{32} \)?
       \[
        \bm{A} = \begin{bmatrix}
          2 & -1 & 0 & 0 \\
          -1  & 2 & -1 & 0 \\
    ...
    \minor{\begin{enumerate}
        \item[34.] Multiply these matrices in the orders \tbm{EF} and \tbm{FE}
          and \(\bm{E}^2\):

        \item[35.] \( \downarrow \)
        \begin{enumerate}
          \item Suppose all columns of \tbm{B} are the same. Then all columns of \tbm{EB}
            are the same, because each one is \tbm{E} times \aset{<?>}.
      ...
      \item[42.] True of false?
        \begin{enumerate}
          ...
          \item If \tbm{AB} and \tbm{BA} are defined, then \tbm{A} and \tbm{B} are square.

          \item If \tbm{AB} and \tbm{BA} are defined, then \tbm{AB} and \tbm{BA} are square.
