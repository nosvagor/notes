...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \minor{\item[13.] Find the dimensions of:
      \begin{enumerate}
        \item the column space of \tbm{A}: \aset{2}
        \item the column space of \tbm{U}: \aset{2}
        \item the row space of \tbm{A}: \aset{2}
        \item the row space of \tbm{U}. \aset{2}
    \end{enumerate}
    \[%%%%%%%%%%
    \bm{A} =
    \begin{bmatrix}
...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \end{bmatrix}_{(e)}
    \]%%%%%%%%%%
    \minor{\item[34.] The transformation \tbm{T} that transposes every matrix is
      definitely linear. Which of these extra properties are true?
      \begin{enumerate}
        \item \(\bm{T}^2\) = identity transformation.
          \begin{itemize}\color{foreground}
            \item \true{true}; given \(\bm{T}(\bm{A}) = \bm{A}^T \)
              \begin{align*}
                \bm{T}^2(\bm{A}) = \bm{T}(\bm{T}(\bm{A})) = \bm{T}(\bm{A}^T ) =
                (\bm{A}^T )^T &= \bm{A} \\
                \then \bm{T}^2 &= \bm{I}
              \end{align*}
          \end{itemize}
        \item The kernel of \(\bm{T}\) is the zero matrix.
        \begin{itemize}\color{foreground}
          \item \true{true}; only matrix that can take the identity to 0 is the
            trivial case, i.e., when the null space is empty.
        \end{itemize}
        \item Every matrix is in the range of \(\bm{T}\).

          \src{
            \link{https://en.wikipedia.org/wiki/Row_and_column_spaces\#Column_space}{Definition
            of column space, Wikipedia 10/16}
          }
          \begin{itemize}\color{foreground}
            \item \true{true}; the image \prn{range} consists of all possible products
              \(\bm{A}\bm{x}~\forall \bm{x} \in \bm{C}^n\).
          \end{itemize}
    ...
    \minor{\item[39.] If you keep the same basis vectors but put them in a
      different order, the change-of-basis matrix \tbm{M} is a
      \aset{permutation matrix \tbm{P}}. If you keep the basis vectors in order
      but change their lengths, \tbm{M} is a \aset{diagonal matrix \tbm{D}}.
    }
  \end{enumerate}
